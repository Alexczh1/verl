...
if __name__ == '__main__':
    ...
    parser.add_argument('--local_dir', default='/opt/tiger/verl/dataset/openthoughts')
    parser.add_argument('--train_start', type=int, default=0)
    parser.add_argument('--train_end', type=int, default=20000)

    args = parser.parse_args()

    data_source = 'open-thoughts/OpenThoughts-114k'
    print(f"Loading the {data_source} dataset from huggingface...", flush=True)
    dataset = datasets.load_dataset(data_source, "metadata", trust_remote_code=True)
    ...
    # Avoid datasets.to_parquet() PyArrow compat (use_content_defined_chunking); write via pandas
    train_dataset.to_pandas().to_parquet(os.path.join(local_dir, 'train.parquet'), index=False)
