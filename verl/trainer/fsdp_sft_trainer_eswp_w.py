...
class FSDPSFTTrainer:
    ...
    def training_step(self, batch: TensorDict, ids, loss_ema: np.ndarray, loss_ema_prune: np.ndarray, epoch: int):
        ...
        for micro_batch in micro_batches:
            ...
            if epoch < self.config.trainer.stop_threshold:
                # update loss_ema
                loss_ema_prune[total_ids] = self.config.trainer.beta_1 * loss_ema_prune[total_ids] + \
                    (1-self.config.trainer.beta_1) * cur_loss_vals[total_ids]
                loss_ema[total_ids] = self.config.trainer.beta_2 * loss_ema[total_ids] + \
                    (1-self.config.trainer.beta_2) * cur_loss_vals[total_ids]
            t1 = time.perf_counter()
            ft += t1 - t0
            loss.backward()
            t2 = time.perf_counter()
            bt += t2 - t1
        self.train_dataloader.sampler.sampler.loss_ema = loss_ema_prune
        ...
        return {
            "train/loss": round(step_loss.detach().item(), 3),
            "train/lr(1e-4)": round(lr * 1e4, 3),
            "train/time(s)": round(spend_time_per_step, 3),
    ...
    def fit(self):
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            loss_ema_prune = np.ones_like(loss_ema) / len(loss_ema)
            ...
            for step_in_epoch, (ids, data) in enumerate(
                ...
                metric = self.training_step(data, ids, loss_ema, loss_ema_prune, epoch)
                if rank == 0:
